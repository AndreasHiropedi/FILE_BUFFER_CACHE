...
{
	// check the offset value is in not negative
	assert(offset >= 0);
	// If we have a cache hit, just return true
	if (cache.retrieve(buffer, offset)) 
	{
		ata_log.messagef(LogLevel::DEBUG, "cache: [CACHE HIT] offset = %d", offset);
		return true;
	}
	// Otherwise, if the transfer works, acknowledge the miss and return true
	if (transfer(0, offset, buffer, count)) 
	{
		ata_log.messagef(LogLevel::DEBUG, "cache: [CACHE MISS] offset = %d", offset);
		cache.insert(buffer, offset);
		return true;
	}
